...
const createExercisePlan = async (selectedExercise) => {
  ...
  exerciseObj.forEach((exercise) => {
    ...
    exerciseItem.classList.add('exercise-item', 'bg-gray-800', 'p-4', 'rounded-md', 'm-6', "text-white", "text-center", "color");
  
    for (const key in exercise) {
      const value = exercise[key];
  
      const exerciseCard = document.createElement('div');
      exerciseCard.classList.add('exercise-card', 'bg-gray-900', 'p-4', 'rounded-md', 'mb-4');
